...
- (void)viewDidLoad
{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(advanceBoardSearch:) name:@"BOARD_ADVANCE_SEARCH" object:nil];
}
...
- (void)viewWillDisappear:(BOOL)animated{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver:self name:@"BOARD_ADVANCE_SEARCH" object:nil];
}
...
- (void)advanceBoardSearch:(NSNotification*)notification{
...
- (void)advanceBoardSearchCall:(NSDictionary*)dataDict{
    ...
    if(selectedtab == 1){
        [myBordAr removeAllObjects];
        iscallback = 3;
        [[SavePrefrences sharedInstance].skthand emitData:@"MyBoard" :@"advncSrchMyBd" :@"":@"":advSearchParams :self];
    }else if(selectedtab == 2){
        sharedAr=[[NSMutableArray alloc]init];
        iscallback = 4;
        [[SavePrefrences sharedInstance].skthand emitData:@"SharedBoard" :@"advShareSrch" :@"":@"":advSearchParams :self];
    }else if(selectedtab == 3){
        commAr=[[NSMutableArray alloc]init];
        iscallback = 5;
        [[SavePrefrences sharedInstance].skthand emitData:@"ComnBoard" :@"advnceSrchAll" :@"":@"":advSearchParams :self];
    }
}
...
-(void)showSharedBord
{
    ...
    search_lbl.delegate=self;
     [search_lbl addTarget:self action:@selector(sharedSimpleSearch) forControlEvents:UIControlEventEditingChanged];
    [middle_label addSubview:search_lbl];
    
    float wid=[SavePrefrences sharedInstance].screenWidth-search_lbl.frame.size.width+5;
    wid=(wid-30)/4;
    
    UIImageView *searchbutton=[[UIImageView alloc]initWithFrame:CGRectMake(search_lbl.frame.size.width+search_lbl.frame.origin.x+5, 5, 25,25)];
    ...
    UIButton *advance=[[UIButton alloc]initWithFrame:CGRectMake(searchbutton.frame.size.width+searchbutton.frame.origin.x+15, 0, 45, middle_label.frame.size.height+8)];
...
-(void) MyboardView
{
    ...
    search_lbl.leftView = paddingView;
    search_lbl.leftViewMode = UITextFieldViewModeAlways;
     [search_lbl addTarget:self action:@selector(search) forControlEvents:UIControlEventEditingChanged];
    search_lbl.text = nil;
    advSearchParams = nil;
    ////////////
    ...
    UIImageView *searchbutton=[[UIImageView alloc]initWithFrame:CGRectMake(search_lbl.frame.size.width+search_lbl.frame.origin.x+5.7,5,25,25)];
...
-(void)sharedSimpleSearch1
{
    ...
    [dic1 setObject:search_lbl.text forKey:@"title"];
...
-(void)showCommunityBoardView
{
    ...
    search_lbl=[[UITextField alloc]initWithFrame:CGRectMake(5, 5,middle_label.frame.size.width/2 +60, middle_label.frame.size.height-7)];
    search_lbl.backgroundColor=[UIColor colorWithRed:225.0f/255 green:232.0f/255 blue:239.0f/255 alpha:1.0f];
    search_lbl.textColor=[UIColor colorWithRed:153.0f/255 green:153.0f/255 blue:153.0f/255 alpha:1.0f];
    ...
    search_lbl.delegate=self;
    [search_lbl addTarget:self action:@selector(CommunitySearch) forControlEvents:UIControlEventEditingChanged];
    [middle_label addSubview:search_lbl];
...
-(void)CommunitySearch{
    ...
    NSLog(@"s1===%@",search_lbl.text);
    NSLog(@"s===%@",search_lbl.text);
