...
-(void)createNode:(NSMutableDictionary *)parentdict andSuperView:(TreeNode *)treenode
{
    ...
    {
        if([parentdict valueForKey:@"folder_id"] )
        {
            
            NSString *str = [parentdict valueForKey:@"folder_id"];
            int count = (int)[[parentdict valueForKey:@"folder_id"] count];
            for (int y = 0; y<count; y++) {
               // NSLog(@"%@",[[parentdict valueForKey:@"name"] objectAtIndex:y]);
                node1 = [[TreeNode alloc] initWithValue:[[parentdict valueForKey:@"folder_id"] objectAtIndex:y]];
                node1.category_id = [[[parentdict valueForKey:@"folder_id"] objectAtIndex:y] intValue];
                node1.parent_id = [[[parentdict valueForKey:@"parent_fid"] objectAtIndex:y] intValue];
                node1.value =[[parentdict valueForKey:@"folder_name"] objectAtIndex:y];
...
//    NSLog(@"%@",indexPath.description);
    TreeNode *node = [[treeNode flattenElements] objectAtIndex:indexPath.row + 1];
    for (TreeNode *node1 in [treeNode flattenElements]) {
        node1.isSelected=NO;
    }
