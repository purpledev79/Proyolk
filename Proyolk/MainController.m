...
- (void)viewDidLoad {
    NSLog(@"Login User Id=============== %@",[SavePrefrences sharedInstance].uid);
    
    // patch by kalpesh
    
    
    if ([SavePrefrences sharedInstance].uid != nil  && 0) {
...
//
    }else if((int)[btn.view tag]==3){
        UIAlertView * alrt=[[UIAlertView alloc]initWithTitle:@"Proyolk" message:@"Coming Soon..." delegate:self cancelButtonTitle:nil otherButtonTitles:@"OK", nil];
        [alrt show];
...
-(void)signIn
{
    ...
    NSString *testVar = @"kalpeshsignin";
    
    NSLog(@"Value of hello = %@", testVar);
...
-(void)login
{
  ...
  //  UIView *outerview= [[UIView alloc] initWithFrame:CGRectMake(20,screenHeight/2,screenWidth-40,screenHeight/2-100)];
    ...
    UIView* fbView = [[UIView alloc] initWithFrame:CGRectMake(10, 40, (outerview.frame.size.width/4)-10, 44)];
    ...
    UIImageView* fbIcon = [[UIImageView alloc] initWithFrame:CGRectMake((fbView.frame.size.width/2)-12, 10, 20, 20)];
...
//    [fbView addSubview:fbLbl];
    ...
    //Linked In Login View
    
    UIView* linkedinView = [[UIView alloc] initWithFrame:CGRectMake((fbView.frame.size.width+20), 40, (outerview.frame.size.width/4)-10, 44)];
    [linkedinView setBackgroundColor:[UIColor colorWithRed:0.0f/255 green:119.0f/255 blue:181.0f/255 alpha:1.0]];
    [linkedinView.layer setCornerRadius:3];
    [outerview addSubview:linkedinView];
    
    UIImageView* linkedinIcon = [[UIImageView alloc] initWithFrame:CGRectMake((linkedinView.frame.size.width/2)-12, 2, 40, 40)];
    [linkedinIcon setContentMode:UIViewContentModeScaleAspectFit];
    [linkedinIcon setImage:[UIImage imageNamed:@"in.png"]];
    [linkedinView addSubview:linkedinIcon];
    
    UITapGestureRecognizer* linkedinBtn = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(Loginhere:)];
    [linkedinBtn setNumberOfTapsRequired:1];
    [linkedinBtn setDelegate:self];
    linkedinView.userInteractionEnabled=YES;
    linkedinView.tag = 2;
    [linkedinView addGestureRecognizer:linkedinBtn];
    
    //Googleplus Login View
    UIView* googleplusView = [[UIView alloc] initWithFrame:CGRectMake((fbView.frame.size.width)+(linkedinView.frame.size.width+30), 40, (outerview.frame.size.width/4)-10, 44)];
    [googleplusView setBackgroundColor:[UIColor colorWithRed:211.0f/255 green:72.0f/255 blue:54.0f/255 alpha:1.0]];
    [googleplusView.layer setCornerRadius:3];
    [outerview addSubview:googleplusView];
    
    UIImageView* googleplusIcon = [[UIImageView alloc] initWithFrame:CGRectMake((googleplusView.frame.size.width/2)-12, 2, 40, 40)];
    [googleplusIcon setContentMode:UIViewContentModeScaleAspectFit];
    [googleplusIcon setImage:[UIImage imageNamed:@"google.png"]];
    [googleplusView addSubview:googleplusIcon];
    
    UITapGestureRecognizer* googleplusBtn = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(Loginhere:)];
        [googleplusBtn setNumberOfTapsRequired:1];
        [googleplusBtn setDelegate:self];
        googleplusView.userInteractionEnabled=YES;
        googleplusView.tag = 1;
        [googleplusView addGestureRecognizer:googleplusBtn];
    
   // http://stackoverflow.com/questions/20901375/yahoo-authenticate-fetch-profile-details-ios
   // https://developer.yahoo.com/social/sdk/objectivec/
   // https://developer.yahoo.com/auth/
    
    
    //Client Id======  dj0yJmk9VFFJV0lNQVFabGJXJmQ9WVdrOWNUSlJaVVI2TjJVbWNHbzlNQS0tJnM9Y29uc3VtZXJzZWNyZXQmeD01OQ--
    // Client Secret Key=== 992d4fa08598e980c3c5b412867f6a770393c4c6
    
    UIView* yahooView = [[UIView alloc] initWithFrame:CGRectMake((fbView.frame.size.width)+(googleplusView.frame.size.width)+(linkedinView.frame.size.width+40), 40, (outerview.frame.size.width/4)-10, 44)];
    [yahooView setBackgroundColor:[UIColor colorWithRed:65.0f/255 green:2.0f/255 blue:151.0f/255 alpha:1.0]];
    [yahooView.layer setCornerRadius:3];
    [outerview addSubview:yahooView];
    
    UIImageView* yahooIcon = [[UIImageView alloc] initWithFrame:CGRectMake((googleplusView.frame.size.width/2)-12, 10, 24, 24)];
    [yahooIcon setContentMode:UIViewContentModeScaleAspectFit];
    [yahooIcon setImage:[UIImage imageNamed:@"yahooicon.png"]];
    [yahooView addSubview:yahooIcon];
    
    UITapGestureRecognizer* yahooBtn = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(Loginhere:)];
    [yahooBtn setNumberOfTapsRequired:1];
    [yahooBtn setDelegate:self];
    yahooView.userInteractionEnabled=YES;
    yahooView.tag = 3;
    [yahooView addGestureRecognizer:yahooBtn];
...
//        [appDel.navigationController setViewControllers:@[landing] animated:NO];
        ...
        AppDelegate* appDel = (AppDelegate*)[[UIApplication sharedApplication] delegate];
        UIStoryboard *mainStoryboard  = [UIStoryboard storyboardWithName:@"Main" bundle:nil];
        MainController* landing = [mainStoryboard instantiateViewControllerWithIdentifier:@"LOGIN_VIEW"];
        [appDel.navigationController setViewControllers:@[landing] animated:NO];
...
-(IBAction)signuptoserver:(id)sender
{
   ...
   if([fname.text isEqual:@""] || [lname.text isEqual:@""] || [username.text isEqual:@""] || [password.text isEqual:@""] || [confirmpass.text isEqual:@""])
    {
